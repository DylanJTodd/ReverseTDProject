...
public class SpawningTowers : MonoBehaviour
{
    ...
    private Transform FindOptimalSpawnPoint(GameObject towerPrefab)
    {
        ...
        Vector3 monsterCentroid = CalculateMonsterCentroid(monsters);
        float maxMapDistance = CalculateMaxMapDistance();

        foreach (Transform point in spawnHolder.transform)
        {
            if (IsSpawnPointOccupied(point)) continue;

            float coverageScore = spawnPointPathCoverage[point] * 0.25f;
            float proximityPenalty = CalculateProximityPenalty(point) * 0.1f;

            float distanceToMonsters = Vector3.Distance(point.position, monsterCentroid);
            float normalizedDistance = distanceToMonsters / maxMapDistance;

            float proximityScore = Mathf.Pow(1 - normalizedDistance, 2) * 0.65f;

            float finalScore = coverageScore + proximityScore - proximityPenalty;
    ...
    private Vector3 CalculateMonsterCentroid(List<Monster> monsters)
    {
        if (monsters.Count == 0) return Vector3.zero;

        Vector3 centroid = Vector3.zero;
        foreach (Monster monster in monsters)
        {
            centroid += monster.transform.position;
        }

        return centroid / monsters.Count;
    }

    private float CalculateMaxMapDistance()
    {
        if (pathsContainer == null || pathsContainer.transform.childCount == 0)
            return 100f;
        ...
        foreach (Transform pathTransform in pathsContainer.transform)
        {
            minBounds = Vector3.Min(minBounds, pathTransform.position);
            maxBounds = Vector3.Max(maxBounds, pathTransform.position);
        }

        return Vector3.Distance(minBounds, maxBounds);
    }
